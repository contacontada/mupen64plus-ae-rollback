...
static void audio_plugin_push_samples(void* aout, const void* buffer, size_t size)
{
    ...
    if (main_rollback_should_suppress_audio()) {
        /* Still advance AI DMA bookkeeping (the caller/RCP state expects
         * this DMA to have "happened"), just don't hand the samples to
         * the audio plugin - a hidden re-simulation frame's audio must
         * never reach the speakers. */
        return;
    }
