...
/* ------------------------------------------------------------------------
 * Rollback netcode: in-memory savestate buffers (ported from RMG-K's
 * savestates_save_rollback_buffer/savestates_load_rollback_buffer, adapted
 * to this core's own savestate binary layout, which is not byte-identical
 * to RMG-K's newer upstream).
 *
 * A rollback buffer is: [4-byte magic "RLBK"][4-byte frame][8-byte
 * reserved][ ...exact same bytes normally written to a .st file, minus
 * gzip compression... ]. No compression is used because rollback saves can
 * happen every frame and must be fast; GekkoNet is responsible for freeing
 * buffers it no longer needs via M64CMD_ROLLBACK_FREE_STATE.
 * ------------------------------------------------------------------------ */
static const unsigned char rollback_buffer_magic[4] = { 'R', 'L', 'B', 'K' };
enum { ROLLBACK_BUFFER_HEADER_SIZE = 16 };

/* Set while savestates_save_m64p()/savestates_load_m64p() are being called
 * for a rollback buffer instead of a real file. */
static unsigned char** rollback_save_out_buffer = NULL;
static int* rollback_save_out_len = NULL;
static unsigned int* rollback_save_out_checksum = NULL;
static int rollback_save_frame = 0;

static const unsigned char* rollback_load_in_buffer = NULL;
static size_t rollback_load_in_len = 0;
...
static int savestates_load_m64p(struct device* dev, char *filepath)
{
    ...
    if (rollback_load_in_buffer != NULL)
    {
        /* Rollback buffer load: read straight from memory, no gzip. */
        const unsigned char* in = rollback_load_in_buffer;
        size_t expected_len = ROLLBACK_BUFFER_HEADER_SIZE + 44 + 16788244 + sizeof(queue) + sizeof(using_tlb_data) + sizeof(data_0001_0200);

        if (rollback_load_in_len != expected_len || memcmp(in, rollback_buffer_magic, 4) != 0)
        {
            main_message(M64MSG_STATUS, OSD_BOTTOM_LEFT, "Rollback state buffer is invalid or the wrong size.");
            SDL_UnlockMutex(savestates_lock);
            return 0;
        }

        memcpy(header, in + ROLLBACK_BUFFER_HEADER_SIZE, 44);
        curr = header;

        if (strncmp((char *)curr, savestate_magic, 8) != 0)
        {
            main_message(M64MSG_STATUS, OSD_BOTTOM_LEFT, "Rollback state buffer is not a valid Mupen64plus savestate.");
            SDL_UnlockMutex(savestates_lock);
            return 0;
        }
        curr += 8;
        ...
        {
            main_message(M64MSG_STATUS, OSD_BOTTOM_LEFT, "Rollback state version (%08x) isn't compatible.", version);
            SDL_UnlockMutex(savestates_lock);
            return 0;
        }
        ...
        {
            main_message(M64MSG_STATUS, OSD_BOTTOM_LEFT, "Rollback state ROM MD5 does not match current ROM.");
            SDL_UnlockMutex(savestates_lock);
            return 0;
        }
        curr += 32;
        ...
        {
            main_message(M64MSG_STATUS, OSD_BOTTOM_LEFT, "Insufficient memory to load rollback state.");
            SDL_UnlockMutex(savestates_lock);
            return 0;
        }

        {
            const unsigned char* p = in + ROLLBACK_BUFFER_HEADER_SIZE + 44;
            memcpy(savestateData, p, savestateSize);
            p += savestateSize;
            memcpy(queue, p, sizeof(queue));
            p += sizeof(queue);
            memcpy(using_tlb_data, p, sizeof(using_tlb_data));
            p += sizeof(using_tlb_data);
            memcpy(data_0001_0200, p, sizeof(data_0001_0200));
        }

        rollback_load_counter++;
        SDL_UnlockMutex(savestates_lock);

        /* fall through to the shared parsing code below via goto, since it
         * lives after the normal gzFile branch closes it out */
        goto rollback_parse_savestate;
    }
...
static int savestates_save_m64p(const struct device* dev, char *filepath)
{
    ...
    if (autoinc_save_slot && rollback_save_out_buffer == NULL)
        savestates_inc_slot();
    ...
    if (rollback_save_out_buffer != NULL)
    {
        /* Rollback buffer save: hand the raw, uncompressed state back to
         * the caller synchronously instead of queuing an async gzip write. */
        size_t total_size = ROLLBACK_BUFFER_HEADER_SIZE + save->size;
        unsigned char* out = (unsigned char *)malloc(total_size);
        if (out == NULL)
        {
            main_message(M64MSG_STATUS, OSD_BOTTOM_LEFT, "Insufficient memory for rollback state.");
            free(save->data);
            free(save->filepath);
            free(save);
            return 0;
        ...
        memcpy(out, rollback_buffer_magic, 4);
        out[4] = (unsigned char)((rollback_save_frame >> 24) & 0xff);
        out[5] = (unsigned char)((rollback_save_frame >> 16) & 0xff);
        out[6] = (unsigned char)((rollback_save_frame >> 8) & 0xff);
        out[7] = (unsigned char)(rollback_save_frame & 0xff);
        memset(out + 8, 0, 8); /* reserved */
        memcpy(out + ROLLBACK_BUFFER_HEADER_SIZE, save->data, save->size);

        *rollback_save_out_buffer = out;
        if (rollback_save_out_len != NULL) {
            *rollback_save_out_len = (int)total_size;
        }
        if (rollback_save_out_checksum != NULL) {
            /* Real content checksum (zlib crc32) over the uncompressed
             * state, so peers can compare it for desync detection. */
            *rollback_save_out_checksum = (unsigned int)crc32(0L,
                out + ROLLBACK_BUFFER_HEADER_SIZE, (uInt)save->size);
        }
...
/* ------------------------------------------------------------------------
 * Rollback netcode: public buffer save/load API (see rollback_stubs.h).
 * These are called directly from api/frontend.c's CoreDoCommand dispatch,
 * bypassing the async job queue used by the normal save/load-state feature
 * since rollback needs a synchronous, uncompressed snapshot every frame.
 * ------------------------------------------------------------------------ */
int savestates_save_rollback_buffer(unsigned char** buffer, int* len, unsigned int* checksum, int frame)
{
    int result;
    ...
    rollback_save_out_buffer = buffer;
    rollback_save_out_len = len;
    rollback_save_out_checksum = checksum;
    rollback_save_frame = frame;

    result = savestates_save_m64p(&g_dev, "ROLLBACK");

    rollback_save_out_buffer = NULL;
    rollback_save_out_len = NULL;
    rollback_save_out_checksum = NULL;

    return result;
}

int savestates_save_full_buffer(unsigned char** buffer, int* len, unsigned int* checksum, int frame)
{
    /* This core's rollback buffer format always contains a complete,
     * self-sufficient state snapshot (there is no incremental/delta
     * encoding), so a "full" save is identical to a normal rollback save. */
    return savestates_save_rollback_buffer(buffer, len, checksum, frame);
}
...
int savestates_load_rollback_buffer(unsigned char* buffer, int len, int frame)
{
    ...
    rollback_load_in_buffer = buffer;
    rollback_load_in_len = (size_t)len;

    result = savestates_load_m64p(&g_dev, "ROLLBACK");

    rollback_load_in_buffer = NULL;
    rollback_load_in_len = 0;
