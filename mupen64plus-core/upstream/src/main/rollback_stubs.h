/* rollback_stubs.h - Rollback netcode API declarations for Android.
 * Implementations: savestates.c (state save/load buffers), main.c (frame
 * execution), device/pif/pif.c (input injection), rollback_stubs.c (thin
...
/* Query deterministic mode (used by main.c to bypass real-time frame
 * pacing while rollback re-simulation needs to run frames back-to-back) */
