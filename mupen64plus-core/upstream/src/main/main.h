...
/* Rollback netcode: whether the frame currently being executed should
 * suppress video/audio output (true during hidden re-simulation steps,
 * see main_rollback_run_frame() in main.c). Checked from vi_controller.c
 * and ai_controller.c so re-simulated frames don't flicker the screen or
 * glitch audio. Always returns 0 outside of rollback mode. */
