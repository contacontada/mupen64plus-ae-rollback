...
/* ------------------------------------------------------------------------
 * Rollback netcode: on-demand single-frame execution.
 *
 * Rather than reimplementing a re-entrant CPU execution entry point (risky:
 * the dynarec backend here isn't designed to be paused/resumed mid-function
 * the way RMG-K's fork does it), this reuses the core's own existing,
 * already-tested single-step mechanism: main_advance_one() + g_rom_pause +
 * pause_loop() (see below), which normally back the "frame advance" debug
 * feature. The emulation thread must already be inside run_device()/
 * run_r4300() (i.e. CoreDoCommand(M64CMD_EXECUTE) already issued and the
 * core already paused) before any M64CMD_ROLLBACK_RUN_FRAME call; this
 * function is expected to be called from a different thread than the one
 * running the CPU (e.g. the netplay/rollback thread), and blocks until the
 * requested frame has actually finished executing.
 * ------------------------------------------------------------------------ */
...
static int l_RollbackLastFrameBefore = 0;
static int l_RollbackLastFrameAfter = 0;
static uint32_t l_RollbackLastPcBefore = 0;
static uint32_t l_RollbackLastPcAfter = 0;
static int l_RollbackLastOutputFlags = 0;
static uint64_t l_RollbackLastFrameUs = 0;

/* Must match the M64FRAME_OUTPUT_* bit layout defined in rollback_jni.cpp
 * (mupen64plus-rollback/jni/rollback_jni.cpp). Kept as plain bit values
 * here rather than a shared header since this is a JNI-bridge-specific
 * convention layered on top of CoreDoCommand's plain "int ParamInt", not
 * part of the core's public API surface (m64p_types.h). */
enum {
    ROLLBACK_OUTPUT_VIDEO = 1 << 0,
    ROLLBACK_OUTPUT_AUDIO = 1 << 1
};

/* 0 = no frame in flight / not rollback mode; suppression getters below
 * only return true while a hidden (non-visible) rollback frame is
 * actually executing. */
static int l_RollbackFrameInFlightFlags = -1;

int main_rollback_should_suppress_video(void)
{
    return l_RollbackFrameInFlightFlags >= 0
        && (l_RollbackFrameInFlightFlags & ROLLBACK_OUTPUT_VIDEO) == 0;
}

int main_rollback_should_suppress_audio(void)
{
    return l_RollbackFrameInFlightFlags >= 0
        && (l_RollbackFrameInFlightFlags & ROLLBACK_OUTPUT_AUDIO) == 0;
}
...
/* Lightweight variant of main_advance_one() for the rollback hot path:
 * skips the StateChanged()/OSD notification, since this can fire many
 * times per second (every visible frame, plus every re-simulated frame
 * during a rollback). */
static void rollback_trigger_one_frame(void)
{
    l_FrameAdvance = 1;
    g_rom_pause = 0;
}
...
int main_rollback_run_frame(int output_flags)
{
    ...
    if (l_RollbackExecuteActive && l_RollbackExecuteCallbacks.begin_frame != NULL) {
        if (!l_RollbackExecuteCallbacks.begin_frame(l_RollbackExecuteCallbacks.user_data)) {
            return 0;
        }
    }

    /* Clear any cached controller-input prediction from a previous frame
     * before the PIF is polled for this one. */
    pif_begin_rollback_input_frame();

    l_RollbackLastFrameBefore = l_CurrentFrame;
    l_RollbackLastPcBefore = *r4300_pc(&g_dev.r4300);
    start_ticks = SDL_GetTicks();

    l_RollbackFrameInFlightFlags = output_flags;
    rollback_trigger_one_frame();

    /* Block until the emulation thread has executed exactly this one
     * frame and re-paused itself (see new_frame()/pause_loop()). This
     * function is meant to be called from a thread other than the one
     * running run_device()/run_r4300() - typically the netplay/rollback
     * thread that owns the GekkoNet session. */
    while (g_EmulatorRunning && !g_rom_pause) {
        SDL_Delay(0);
    }
    l_RollbackFrameInFlightFlags = -1;

    end_ticks = SDL_GetTicks();

    l_RollbackLastFrameUs = (uint64_t)(end_ticks - start_ticks) * 1000ULL;
    l_RollbackLastFrameAfter = l_CurrentFrame;
    l_RollbackLastPcAfter = *r4300_pc(&g_dev.r4300);
    l_RollbackLastOutputFlags = output_flags;

    if (!g_EmulatorRunning) {
        return 0;
    }

    if (l_RollbackExecuteActive && l_RollbackExecuteCallbacks.end_frame != NULL) {
...
void main_get_rollback_run_frame_stats(m64p_rollback_run_frame_stats* stats)
{
    ...
    /* Only the fields this core can actually measure without deeper
     * dynarec/plugin instrumentation are filled in; the rest (per-phase
     * timing breakdown, dynarec recompile counters, etc. - fields RMG-K's
     * newer core tracks internally) are left at zero rather than faked. */
    stats->total_us = l_RollbackLastFrameUs;
    stats->r4300_us = l_RollbackLastFrameUs;
    stats->emumode = (uint64_t)g_dev.r4300.emumode;
    stats->cp0_count_before = 0;
    stats->cp0_count_after = 0;
    stats->pc_before = l_RollbackLastPcBefore;
    stats->pc_after = l_RollbackLastPcAfter;
    stats->current_frame_before = l_RollbackLastFrameBefore;
    stats->current_frame_after = l_RollbackLastFrameAfter;
    stats->output_flags = l_RollbackLastOutputFlags;
}
...
static void apply_speed_limiter(void)
{
    ...
    if (rollback_is_deterministic_mode()) {
        /* Rollback re-simulation must run as fast as the CPU allows, not
         * paced to wall-clock time - GekkoNet/main_rollback_run_frame's
         * caller is responsible for pacing visible frames instead. */
        resetOnce = 0;
        return;
    }
