...
EXPORT m64p_error CALL CoreDoCommand(m64p_command Command, int ParamInt, void *ParamPtr)
{
    ...
    switch(Command)
    {
        ...
        /* Rollback netcode commands (from RMG-K) */
        case M64CMD_ROLLBACK_SAVE_STATE:
            if (ParamPtr == NULL)
                return M64ERR_INPUT_ASSERT;
            if (((m64p_rollback_state*)ParamPtr)->checksum == 0x46554c4c) /* 'FULL' */
        ...
        case M64CMD_ROLLBACK_FREE_STATE:
            if (ParamPtr == NULL)
                return M64ERR_INPUT_ASSERT;
            savestates_free_rollback_buffer((unsigned char*)ParamPtr);
            return M64ERR_SUCCESS;
        case M64CMD_ROLLBACK_SET_INPUT_CALLBACK:
            savestates_set_rollback_input_callback((m64p_rollback_input_callback)ParamPtr);
            return M64ERR_SUCCESS;
        case M64CMD_ROLLBACK_SET_INPUT_PLAYERS:
            savestates_set_rollback_input_players(ParamInt);
            return M64ERR_SUCCESS;
        case M64CMD_ROLLBACK_SET_DETERMINISTIC:
            savestates_set_rollback_deterministic(ParamInt);
            return M64ERR_SUCCESS;
        case M64CMD_ROLLBACK_SAMPLE_INPUT:
            if (ParamPtr == NULL)
                return M64ERR_INPUT_ASSERT;
            return savestates_rollback_sample_input(
                ((m64p_rollback_input_sample*)ParamPtr)->values,
                ((m64p_rollback_input_sample*)ParamPtr)->size,
                ((m64p_rollback_input_sample*)ParamPtr)->players) ? M64ERR_SUCCESS : M64ERR_INTERNAL;
        ...
        case M64CMD_ROLLBACK_SET_VERBOSE_STATS:
            savestates_set_rollback_verbose_stats(ParamInt);
            return M64ERR_SUCCESS;
