...
> **Versão atual:** Patch 51. Esta atualização mantém as correções dos Patches 38, 39, 41, 42, 43, 44, 46, 47, 48, 49 e 50 e amplia o diagnóstico UDP com contadores de entrada nas funções de envio e recepção, além de emitir o resumo periódico também durante o loop de espera por eventos. A correção de concorrência entre o loop normal do `CoreService` e a execução controlada pelo rollback permanece ativa. O fluxo de inicialização continua usando o lançamento da `GameActivity` por nome de componente, compatível com a separação entre os módulos `app` e `mupen64plus-rollback`.
