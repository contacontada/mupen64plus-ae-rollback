...
> **Versão atual:** Patch 44. Esta atualização mantém as correções dos Patches 38, 39, 41, 42 e 43 e acrescenta um marcador nativo inequívoco na entrada de `nativeExecute()`, permitindo confirmar nos logs que a biblioteca nativa carregada corresponde ao build atual. O fluxo de inicialização continua usando o lançamento da `GameActivity` por nome de componente, compatível com a separação entre os módulos `app` e `mupen64plus-rollback`.
