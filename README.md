...
> **Versão atual:** Patch 43. Esta atualização mantém as correções dos Patches 38, 39, 41 e 42 e acrescenta diagnóstico de entrada por frame, registrando separadamente as chamadas do callback durante frames ocultos e visíveis para facilitar a identificação de falhas de sincronização. Também torna o lançamento da `GameActivity` no serviço de rollback explicitamente tipado, preservando a separação entre processos e a compatibilidade do fluxo de inicialização.
