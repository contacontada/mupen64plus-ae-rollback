...
> **Versão atual:** Patch 47. Esta atualização mantém as correções dos Patches 38, 39, 41, 42, 43, 44 e 46 e corrige a concorrência entre o loop normal do `CoreService` e a execução controlada pelo rollback, fazendo o serviço aguardar enquanto a sessão rollback estiver ativa. Também preserva o diagnóstico nativo ampliado com contadores de eventos e callbacks. O fluxo de inicialização continua usando o lançamento da `GameActivity` por nome de componente, compatível com a separação entre os módulos `app` e `mupen64plus-rollback`.
