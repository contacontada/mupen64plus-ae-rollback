...
> **Versão atual:** Patch 46. Esta atualização mantém as correções dos Patches 38, 39, 41, 42, 43 e 44 e amplia o diagnóstico nativo do rollback com contadores de eventos, registros de `GekkoSaveEvent` e `GekkoAdvanceEvent` e um resumo periódico das chamadas de callback, facilitando a identificação de falhas de sincronização e de avanço de frames. O fluxo de inicialização continua usando o lançamento da `GameActivity` por nome de componente, compatível com a separação entre os módulos `app` e `mupen64plus-rollback`.
