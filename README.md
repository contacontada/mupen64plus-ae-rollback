...
> **Versão atual:** Patch 53. Esta atualização mantém as correções dos Patches 38, 39, 41, 42, 43, 44, 46, 47, 48, 49, 50, 51 e 52, preserva os diagnósticos UDP, mantém a compatibilidade do lobby RMG-K 0.9.14 e amplia o buffer interno de diagnóstico nativo para 1024 bytes. A correção de concorrência entre o loop normal do `CoreService` e a execução controlada pelo rollback permanece ativa. O fluxo de inicialização continua usando o lançamento da `GameActivity` por nome de componente, compatível com a separação entre os módulos `app` e `mupen64plus-rollback`.
